...
## CAEP-Naming-Drift — interner Rename (Decision Required, aus 30.05.2026)
- **Problem:** Code/Endpoints/Docstrings nutzen Namen "CAEP" + "Continuous Access Evaluation" = direkter Overlap mit OpenID-CAEP-Spec, ABER MolTrust fährt proprietäres pending/acknowledge-Polling (NICHT SET/RFC 8417, kein SSF-Stream/Transmitter/Receiver). caep.py-Docstring dokumentiert das selbst ehrlich, aber Name nach außen ist irreführend.
- **30.05. erledigt (Quick-Fix):** Card-Extension https://moltrust.ch/extensions/caep/v1 disambiguiert öffentlich ("name-inspired by OpenID-CAEP but NOT SET/RFC 8417 compliant"). Discovery-Ebene sauber.
- **Offen (verbindlich, nicht optional):** Interner Rename auf eigenständigen Namen (z.B. "MolTrust Event Notifications") — Code + Endpoints /caep/* + DB-Tabellen + Docstrings. Trust-Vendor sollte keinen fremden Spec-Namen für proprietäres Protokoll tragen. Eigener Sprint, mehrere PRs. NICHT vor JWS-Sprint, aber vor breiterem Marketplace-Push.
- **Nur falls Marketplace-Reviewer echtes OpenID-CAEP fordert:** statt Rename echte SET/SSF-Compliance bauen (großer Sprint) — unwahrscheinlich, erst bei expliziter Forderung.

## Changelog

- **2026-05-30 — V1.16**: CAEP-Naming-Drift dokumentiert — Card-Disambiguation live (Quick-Fix), interner Rename (Code/Endpoints/DB) als verbindliches Item offen. Trust-Vendor trägt keinen fremden Spec-Namen für proprietäres Protokoll.
