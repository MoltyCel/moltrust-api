...
async def ensure_aws_marketplace_tables(conn):
    ...
    # detail.agreement.id — the only identifier all four EventBridge event
    # types carry. ResolveCustomer does not return it, so it is stamped on
    # from the first license event rather than written at fulfillment.
    await conn.execute(
        "ALTER TABLE aws_marketplace_subscribers "
...
async def apply_license_event(conn, detail_type, acceptor_account_id, license_arn,
                              product_code, event_time, agreement_id=None):
    """Apply one EventBridge event to the subscriber rows it identifies.

    Correlation is a cascade, forced by what the real events actually carry
    (verified against the four captured on 2026-08-20):

      agreement.id   all four event types
      license.arn    License Updated / License Deprovisioned only
      product        License events only — the agreement events have none
      acceptor       all four

    ResolveCustomer returns neither an agreement id nor anything else linking a
    subscriber row to one, so license.arn is the bridge: the first license event
    stamps agreement_id onto the row its ARN identifies, and every later event
    for that agreement matches on the id directly.

    A match therefore requires an agreement id or a license ARN. The account
    alone is never enough — one account can hold several concurrent agreements,
    and matching on it would move all of them together.

    ARNs are compared on the trailing identifier: ResolveCustomer returns
    arn:aws:license-manager::294406891311:license:l-9d01... while the documented
    form is aws:license-manager:us-east-1:...:l-e52c... — different prefix,
    different region position, different separator.

    Returns (matched_rows, updated_rows); updated is 0 for a stale delivery.
    """
    ...
    # Stamp the agreement onto whatever the ARN identifies, so the agreement
    # events that carry no ARN can correlate from here on.
    if agreement_id and tail:
        await conn.execute(
            """
            UPDATE aws_marketplace_subscribers
               SET agreement_id = $1
             WHERE agreement_id IS NULL
               AND customer_aws_account_id = $2
               AND license_arn LIKE '%' || $3
            """,
            agreement_id, acceptor_account_id, tail,
        )

    rows = await conn.fetch(
        """
        SELECT id FROM aws_marketplace_subscribers
         WHERE customer_aws_account_id = $1
           AND ($2::text IS NULL OR product_code = $2)
           AND ( ($3::text IS NOT NULL AND agreement_id = $3)
              OR ($4::text IS NOT NULL AND license_arn LIKE '%' || $4) )
        """,
        acceptor_account_id, product_code, agreement_id, tail,
    )
    if not rows:
        return 0, 0
    ...
    updated = await conn.fetch(
        ...
        UPDATE aws_marketplace_subscribers
         ...
         WHERE id = ANY($1::bigint[])
           AND (status_updated_at IS NULL OR status_updated_at < $3)
        RETURNING id
        """,
        ids, status, event_time,
    )
    return len(ids), len(updated)


async def replay_unmatched_events(conn, account_id, license_arn, product_code):
    """Apply EventBridge events that arrived before this subscriber existed.

    The agreement is created before the buyer ever reaches the registration
    page — on 2026-08-20 License Updated landed 21 seconds ahead of the
    fulfillment POST. Without this the row stays pending forever while the
    licence is live.
    """
    pending = await conn.fetch(
        """
        SELECT event_id, detail_type, acceptor_account_id, license_arn,
               product_code, agreement_id, sns_timestamp
          FROM aws_marketplace_notifications
         WHERE NOT matched
           AND channel = 'eventbridge'
           AND acceptor_account_id = $1
         ORDER BY sns_timestamp NULLS FIRST
        """,
        account_id,
    )
    for ev in pending:
        matched, _ = await apply_license_event(
            conn, ev["detail_type"], ev["acceptor_account_id"], ev["license_arn"],
            ev["product_code"], ev["sns_timestamp"], ev["agreement_id"],
        )
        if matched:
            await conn.execute(
            ...
            log.info("replayed %s (%s) for account %s",
                     ev["event_id"], ev["detail_type"], account_id)
...
async def _persist_subscriber(conn, account_id, customer_identifier, license_arn, product_code):
    ...
    if license_arn:
        # Only an ARN-less row from the SAME registration attempt may be filled.
        # Without the time bound this grabbed a row created 34 hours earlier for
        # a different agreement (2026-08-19 row, 2026-08-20 licence) and quietly
        # repurposed it. An hour is far longer than a registration takes and far
        # shorter than the gap between two purchases.
        await conn.execute(
            """
            UPDATE aws_marketplace_subscribers
               SET license_arn = $3
    ...
    # EventBridge first — the agreement predates the registration.
    await replay_unmatched_events(conn, account_id, license_arn, product_code)
    if customer_identifier:
        await _replay_unmatched(conn, customer_identifier, product_code)

    # Re-read unconditionally: a replay may just have moved this row to active,
    # and the status returned by the upsert above predates it. Doing this only
    # for customer_identifier — which AWS never populates for this listing —
    # meant the caller always saw the pre-replay value.
    row = await conn.fetchrow(
        """
