...
async def reseller_env(app_with_lifespan):
    ...
    async def login_token(login, password):
        r = await client.post("/reseller/login", json={"login": login, "password": password})
...
async def test_login_and_generic_failure(reseller_env):
    ...
    assert tok
    bad = await reseller_env.client.post("/reseller/login", json={"login": r["login"], "password": "wrong"})
    assert bad.status_code == 401
    nouser = await reseller_env.client.post("/reseller/login", json={"login": "nope_" + uuid.uuid4().hex[:6], "password": "x"})
...
async def test_routes_require_auth(reseller_env):
    for path in ("/reseller/me", "/reseller/agents", "/reseller/billing"):
        r = await reseller_env.client.get(path)
        assert r.status_code == 401, f"{path} should be 401 without token"
    r = await reseller_env.client.get("/reseller/agents", headers={"Authorization": "Bearer garbage"})
...
async def test_tenant_isolation(reseller_env):
    ...
    for d in a_dids:
        r = await reseller_env.client.post("/reseller/agents", json={"did": d}, headers={"Authorization": f"Bearer {ta}"})
        assert r.status_code == 200, r.text
    for d in b_dids:
        r = await reseller_env.client.post("/reseller/agents", json={"did": d}, headers={"Authorization": f"Bearer {tb}"})
        assert r.status_code == 200, r.text

    ra = await reseller_env.client.get("/reseller/agents", headers={"Authorization": f"Bearer {ta}"})
    rb = await reseller_env.client.get("/reseller/agents", headers={"Authorization": f"Bearer {tb}"})
    ...
    ba = (await reseller_env.client.get("/reseller/billing", headers={"Authorization": f"Bearer {ta}"})).json()
    bb = (await reseller_env.client.get("/reseller/billing", headers={"Authorization": f"Bearer {tb}"})).json()
...
async def test_did_double_assignment_409(reseller_env):
    ...
    r1 = await reseller_env.client.post("/reseller/agents", json={"did": d}, headers={"Authorization": f"Bearer {ta}"})
    assert r1.status_code == 200 and r1.json()["status"] == "created"
    # A re-onboards same DID -> idempotent
    r1b = await reseller_env.client.post("/reseller/agents", json={"did": d}, headers={"Authorization": f"Bearer {ta}"})
    ...
    r2 = await reseller_env.client.post("/reseller/agents", json={"did": d}, headers={"Authorization": f"Bearer {tb}"})
...
async def test_invalid_did_rejected(reseller_env):
    a = await reseller_env.mk_reseller()
    ta = await reseller_env.login_token(a["login"], a["password"])
    r = await reseller_env.client.post("/reseller/agents", json={"did": "not-a-did"}, headers={"Authorization": f"Bearer {ta}"})
...
async def test_logout_revokes(reseller_env):
    a = await reseller_env.mk_reseller()
    ta = await reseller_env.login_token(a["login"], a["password"])
    ok = await reseller_env.client.get("/reseller/me", headers={"Authorization": f"Bearer {ta}"})
